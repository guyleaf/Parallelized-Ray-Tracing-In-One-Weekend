...
void write_color(std::ostream &out, color pixel_color, int samples_per_pixel)
{
    ...
    // gamma=2.0.
    auto scale = 1.0_r / samples_per_pixel;
    r = sqrt(scale * r);
    g = sqrt(scale * g);
    b = sqrt(scale * b);

    // Write the translated [0,255] value of each color component.
    out << static_cast<int>(256 * clamp(r, 0.0_r, 0.999_r)) << ' '
        << static_cast<int>(256 * clamp(g, 0.0_r, 0.999_r)) << ' '
        << static_cast<int>(256 * clamp(b, 0.0_r, 0.999_r)) << '\n';
}
